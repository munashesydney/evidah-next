...
/**
 * PUT /api/inbox/ticket/ai-suggestion/clear
 * Clears the AI suggestion for a ticket
 * 
 * Body parameters:
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Update ticket to clear AI suggestion
    ...
    return NextResponse.json({
      status: 1,
      message: 'AI suggestion cleared successfully',
      ticketId,
    });
  } catch (error: any) {
    ...
    return NextResponse.json(
      {
        status: 0,
        message: 'Error clearing AI suggestion',
        error: error.message,
      },
      { status: 500 }
    );
