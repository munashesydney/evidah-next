...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch features settings', details: error.message },
      { status: 500 }
    );
  }
}

// PUT - Update features settings
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    const { uid, selectedCompany = 'default', showAvatar, showTypingIndicator, showOnlineStatus } = body;
    ...
    if (showTypingIndicator !== undefined) updateData.showTypingIndicator = showTypingIndicator;
    ...
    console.error('Error updating features live chat settings:', error);
    return NextResponse.json(
      { error: 'Failed to update features settings', details: error.message },
      { status: 500 }
    );
  }
}
