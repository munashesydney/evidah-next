...
export default function AdvancedSettingsPage() {
  ...
  const fetchSettings = async (userId: string) => {
    try {
      const response = await fetch(
        `/api/settings/live-chat/advanced?uid=${userId}&selectedCompany=${selectedCompany}`
      );
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/api/settings/live-chat/advanced', {
        method: 'PUT',
        headers: {
    ...
    <div className="grow">
      <div className="p-6 space-y-6">
        <div>
          ...
          <div className="text-sm">Add custom CSS to further customize your live chat widget.</div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                Use custom CSS to override default styles. Be careful with your selectors to avoid breaking the widget.
              </div>
            </div>
      ...
      <LiveChatPreview advancedSettings={settings} />
    </div>
  );
}
